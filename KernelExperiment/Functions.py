...
"""
@file Functions.py
@brief EEG sleep staging pipeline using spectrograms and CNNs.

@details
Contains functions and classes for loading, filtering, segmenting, and classifying EEG data for sleep staging.
Uses MNE, NumPy, SciPy, OpenCV, PyTorch, and scikit-learn.

@note
All functions and classes are documented with Doxygen-style comments.
"""

# Parameters based on data
SAMPLING_RATE = 100  # (fs) Hz
"""@var SAMPLING_RATE
@brief Sampling rate of EEG data in Hz.
"""

EPOCH_DURATION = 30  # (seconds) - Standard for sleep staging
"""@var EPOCH_DURATION
@brief Duration of each epoch in seconds.
"""

NPERSEG = 5 * SAMPLING_RATE  # (500 samples) 2-second sub-window for STFT
"""@var NPERSEG
@brief Number of samples per segment for STFT.
"""

NOVERLAP = NPERSEG // 2  # (250 samples) 50% overlap
"""@var NOVERLAP
@brief Number of overlapping samples for STFT.
"""

IMG_SIZE = (128, 128)  # (height, width) Final image size for the CNN
"""@var IMG_SIZE
@brief Size of spectrogram images for CNN input.
"""
...
"""@var stage_map
@brief Mapping of sleep stage names to integer codes.
"""

NUM_CLASSES = 5 # 5 sleep stages
"""@var NUM_CLASSES
@brief Number of sleep stage classes.
"""

device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
"""@var device
@brief PyTorch device for computation ('cuda' or 'cpu').
"""
...
def load_and_filter_eeg(file_path, channel='EEG Fpz-Cz'):
    ...
    @param file_path Path to the EEG EDF file. If None, generates simulated EEG data.
    @param channel Name of the EEG channel to select (default: 'EEG Fpz-Cz').

    @return Tuple containing filtered EEG data (numpy array) and the raw MNE object (if file is loaded).
    """
...
def create_y_labels(annotations, stage_map, epoch_duration, raw):
    ...
    @param annotations MNE Annotations object containing sleep stage events.
    @param stage_map Dictionary mapping sleep stage names to integer codes.
    @param epoch_duration Duration of each epoch in seconds.
    @param raw MNE Raw object containing EEG data.

    @return Numpy array of integer labels for each epoch.
    """
...
def create_epochs(data, epoch_len_samples):
    """
    @brief Chops the data into fixed-length epochs.

    @param data 1D numpy array of EEG data.
    @param epoch_len_samples Number of samples per epoch.

    @return Numpy array of shape (num_epochs, epoch_len_samples).
    """
...
def create_spectrogram_image(epoch_data):
    ...
    @param epoch_data 1D numpy array of EEG data for one epoch.

    @return 2D numpy array representing the resized spectrogram image.
    """
...
def normalize(image):
    """
    @brief Normalizes a single image to Z-score (mean=0, std=1).
...
class EEGDataset(Dataset):
    """
    @brief Custom PyTorch Dataset for EEG spectrogram images and labels.

    @param X Numpy or torch array of input images.
    @param Y Numpy or torch array of integer labels.
    """
    ...
    def __getitem__(self, idx):
        """
        @brief Returns the image and label at the given index.
...
class CNN(nn.Module):
    ...
    @param in_channels Number of channels in the input image (usually 1 for grayscale).
    @param num_classes Number of output classes (sleep stages).
    """
    ...
    def forward(self, x):
        ...
        @return Output tensor of shape (batch, num_classes).
        """
        x = self.features(x)
        x = self.classifier(x)
        return x
...
def train_eeg_model(model, train_loader, val_loader, num_epochs):
    """
    @brief Trains the EEG sleep stage classification model.
...
def test_eeg_model(model, test_loader, device):
    """
    @brief Tests the trained EEG model on the test set.

    @param model Trained PyTorch model.
    @param test_loader DataLoader for test data.
    @param device Device to run inference on ('cpu' or 'cuda').

    @return Test accuracy as a float.
    """
    model.eval()
...
if __name__ == "__main__":
    """
    @brief Main script for running EEG sleep staging pipeline.

    @details
    Loads EEG data, processes epochs, creates spectrogram images, splits data, trains and tests CNN model.
    """
